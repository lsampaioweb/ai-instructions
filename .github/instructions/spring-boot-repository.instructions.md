...
- Use `findById`, `findAll`, `findBy*`, `save`, `update`, `deleteById` as the standard method name prefixes for repository interface methods; keep method names consistent with operation semantics across all repositories.
...
- Build SQL parameter containers to be null-tolerant by omitting null entries or binding typed null values explicitly; never pass raw nulls without explicit SQL type intent.
- Accept pagination parameters (offset, limit, sort column) as explicit method parameters for paginated queries; never embed hardcoded limits inside SQL statements.
