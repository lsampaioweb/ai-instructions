...
- Use this canonical section order for engine instruction files: `## Scope & Analysis`, `## Dependencies` (if applicable), `## Naming Conventions` (if applicable), `## Resolution Rules`, `## Approved Exception Handling` (if applicable), `## Safety Guards`, `## Review Plan Layout`.
...
- Before finalizing an instruction file, read it as if implementing code from it, identify unguided decisions, and add rules for each preference.
