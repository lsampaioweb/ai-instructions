...
- For security role enum naming conventions, defer to `spring-boot-security.instructions.md`.
...
- Treat adding a new enum value to a public API contract as a potentially breaking change for strict consumers; document additions in the API changelog.
- Keep enum names expressive and domain-specific.
- Keep enum-to-string behavior explicit when used in contracts.
- Serialize enums as their `name()` string value in all API and message contracts; never use ordinal-based serialization.
