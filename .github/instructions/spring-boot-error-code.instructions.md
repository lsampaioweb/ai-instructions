...
applyTo: "**/src/main/java/**/*ErrorCode.java"
---
...
- Use UPPER_SNAKE_CASE for all error-code enum values (e.g., `HOLIDAY_NOT_FOUND`, `DUPLICATE_HOLIDAY`).
- Include the resource or domain name as a prefix in every error code value to prevent ambiguity across modules (e.g., `USER_NOT_FOUND`, not `NOT_FOUND`).
...
- Resolve HTTP status codes in the exception handler layer.
...
- Declare error codes only in dedicated catalog classes or handler-local constants.
...
- When the `errorCode` field is exposed in the API payload, format its value as the UPPER_SNAKE_CASE enum constant name.
- Scope each `*ErrorCode` catalog to a single feature.
...
- Create separate `*ErrorCode` catalogs per feature when a module has multiple resources with distinct failure domains.
