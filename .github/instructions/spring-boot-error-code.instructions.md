...
- When retiring an error code, annotate it `@Deprecated` and keep it active for at least one major version before removal; document the replacement code in the deprecation annotation.
...
- Resolve HTTP status codes in the exception handler layer, not inside the error code enum; never embed HTTP status values in error code declarations.
