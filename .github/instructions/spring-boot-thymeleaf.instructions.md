...
- Place reusable layout fragments under `templates/fragments/` and layout templates under `templates/layouts/`; never inline reusable fragments inside feature templates.
- Define a custom `templates/error.html` for application error fallback; never rely on Spring Boot's default white-label error page in production.
- Place CSS in `static/css/`, JavaScript in `static/js/`, and images in `static/images/`; reference static resources using Thymeleaf's `@{/css/...}` link expressions.
- Keep form binding and validation explicit at controller boundaries.
- Use `th:errors` to display field-level validation errors adjacent to each form input; never display all validation errors in a single block without field association.
