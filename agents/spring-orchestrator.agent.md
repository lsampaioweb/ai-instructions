...
7. Require the plan to include `Activated instruction files` and `Acceptance gates`.
8. Before each coder pass, output `Iteration <N> out of 5`.
9. In each iteration, send `spring-coder` only: current task scope, unresolved findings, activated instruction files, and acceptance gates.
10. Require `spring-coder` preflight and post-implementation compliance report for activated instruction files.
11. Do not require code-level "how" details or explicit file read/edit lists.
12. Provide the plan to `spring-coder` to implement it.
13. Run all review subagents in parallel.
14. If problems remain, send only unresolved problems to `spring-architect` to update the plan, then send the updated plan to `spring-coder` for fixes.
15. Re-run only reviewers tied to unresolved findings or changed files by default.
16. Re-run all reviewers only when changed files touch shared config, security, API contracts, or two or more feature areas.
17. After reviewer status is PASS, run `spring-documenter` to synchronize Markdown documentation based on final code/configuration changes.
18. Require `spring-documenter` to present a read-only documentation sync plan before applying doc edits.
19. Stop after 5 iterations, after 2 no-progress cycles, or after 1 unchanged Critical finding.
20. If the loop stops early, require human decision.
