...
- Message keys in properties files must follow the namespace pattern: `validation.*` for request validation errors, `error.*` for domain errors, `openapi.*` for API documentation, and `log.*` for log messages.
