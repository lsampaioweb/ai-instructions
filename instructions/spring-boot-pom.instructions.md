...
For Flyway/Liquibase dependency policy and migration-tool selection, follow `spring-boot-migrations.instructions.md`.
For caching dependency and backend selection policy, follow `spring-boot-caching.instructions.md`.
...
- Set the Java baseline via the `<java.version>` property
...
- **Document every dependency purpose**: add a brief XML comment above each `<dependency>` block explaining why it was added
...
- When using Java 23 or later, explicitly declare Lombok in `<annotationProcessorPaths>` inside `maven-compiler-plugin`; Java 23+ removed implicit annotation processor discovery, so without this `@Slf4j`, `@Data`, and all other Lombok annotations will fail to compile; see `snippets/pom/pom.xml` for the configuration
